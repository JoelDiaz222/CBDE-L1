...
# This script performs similarity search using pgvector with two separate tables
...
for sentence in chosen_sentences:
    ...
    # Euclidean distance using pgvector with two tables
    euclidean_query = """
    SELECT s.id, s.sentence, e.embedding <-> %s::vector AS euclidean_distance
    FROM sentences s
    JOIN embeddings_pgvector e ON s.id = e.id
    ...
    # Cosine distance using pgvector with two tables
    cosine_query = """
    SELECT s.id, s.sentence, e.embedding <=> %s::vector AS cosine_distance
    FROM sentences s
    JOIN embeddings_pgvector e ON s.id = e.id
    ORDER BY e.embedding <=> %s::vector
    LIMIT 2;
